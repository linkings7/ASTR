import math
from lib.models.astr import build_astr
...
class ASTR(BaseTracker):
    def __init__(self, params):
        super(ASTR, self).__init__(params)
        network = build_astr(params.cfg, training=False)
    ...
    def initialize(self, image, info: dict):
        ...
        with torch.no_grad():
            self.init_z_tensor = template
            self.cur_z_tensor = template
    ...
    def update(self, image, bbox):
        # forward the template once
        z_patch_arr, resize_factor, z_amask_arr = sample_target(image, bbox, self.params.template_factor, output_sz=self.params.template_size)
        template = self.preprocessor.process(z_patch_arr)
        with torch.no_grad():
            self.cur_z_tensor = template
    ...
    def track(self, image, info: dict = None):
        ...
        with torch.no_grad():
            ...
            out_dict = self.network.forward(
                template=self.init_z_tensor, search=x_tensor, cz=self.cur_z_tensor)
